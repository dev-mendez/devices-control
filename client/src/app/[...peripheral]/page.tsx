  ...
  return (
    <div className=" bg-white text-gray-500  h-min min-w-25 pb-5 shadow-inner shadow-gray-300">
        ...
        <div className="flex justify-between p-2  text-2xl  border-b-2 shadow shadow-gray-200  border-gray-600">
          ...
          <div className="space-x-2 flex flex-row">
            <Link className="flex align-middle" href={'/'}>
              <button className="px-2 bg-emerald-200 hover:bg-green-400 self-center"><MdOutlineArrowBackIos className="" size={30} /></button>
            </Link>
            <button onClick={toggleModal} className=" px-2 bg-emerald-200 hover:bg-green-400"><p className="">Connect</p></button>
          </div>
        </div>
      </div>
      <div className="w-full h-auto  flex flex-col  p-2  items-center ">
        ...
        {perihperals.map((perihperal, id) => (<Peripheral key={id} props={{ ...perihperal, disconnectPeripheral, changePeripheralStatus, toggleModal }} />))}
